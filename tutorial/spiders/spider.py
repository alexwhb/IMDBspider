...
class tutorialSpider(scrapy.Spider):
	...
	def parse(self, response):
		...
		for sel in response.xpath("//*[contains(@class,'chart full-width')]/tbody/tr"):#//TODO==king it seems that IMDB has changed the html structure for these information
			item = MovieItem()
	...
	def mapFilmDetails(self, response, titleDetails, item, index):
		...
		if(titleDetails):
			if("Language" in titleDetails):
				...
				item['Budget'] =  self.ifNotEmptyGetIndex(response.xpath("//*[@id='titleDetails']/div["+str(index)+"]/text()").extract(),1)
			elif("Gross" in titleDetails):
				item['GrossProfit'] = self.ifNotEmptyGetIndex(response.xpath("//*[@id='titleDetails']/div["+str(index)+"]/text()").extract(),1)
			elif("Opening Weekend" in titleDetails):
				item['OpeningWeekendProfit'] = self.ifNotEmptyGetIndex(response.xpath("//*[@id='titleDetails']/div["+str(index)+"]").extract()) #it also might be just \d
